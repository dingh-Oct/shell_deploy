...
head_color='\033'
tail_color='\033[0m'
log_file="$(cd ../log;pwd)/nginx_all.log"
...
# 安装nginx
install_nginx(){    
    echo -e "$head_color[1;33m正在安装nginx $tail_color"
    wget -q http://nginx.org/en/download/nginx-1.12.2.tar.gz -P /root/
    if [ -f '/root/nginx-1.12.2.tar.gz' ];then
        tar xf ~/nginx-1.12.2.tar.gz -C /usr/local/;cd /usr/local/nginx-1.12.2
        ./configure --prefix=/usr/local/nginx --sbin-path=/usr/bin/nginx &>"$log_file";check_command && \
        make &>"$log_file";check_command && make install &>"$log_file";check_command && \
